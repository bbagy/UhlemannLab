...
class TETyper:
    ...
    def generate_vcf(self):
        ...
        # bcftools mpileup: -Ou (BCF, uncompressed) + -f REF + BAM
        bcftools_mpileup_args = ['bcftools', 'mpileup', '-Ou', '-f', self.ref, self.bam]
        # bcftools call: -mv (variants only) + -Ov (VCF text) + -o <vcf>
        bcftools_call_args = ['bcftools', 'call', '-mv', '-Ov', '-o', self.snpfile]
