...
# 4) MultiQC
# 5) Summary (Rmd: 20251007_Summary_WGS_tem_v3.Rmd)
...
REPORT_RMD  = config.get("report_rmd", "/pipeline/scripts/20251007_Summary_WGS_tem_v3.Rmd")
...
def norm_species_to_code(std_name: str) -> str:
    ...
    synonyms = {
        ...
        "klebsiella quasipneumoniae subsp. quasipneumoniae": "klebsiella pneumoniae",
        "klebsiella quasipneumoniae subsp. similipneumoniae": "klebsiella pneumoniae",
...
# Kraken2 DB 정보 기록 (리포트 표시용)
rule kraken2_db_info:
    output:
        info = f"{OUTPUT_DIR}/2a_kraken2/kraken2_db_info.txt"
    params:
        db = KRAKEN_DB,
        db_host = KRAKEN_DB_HOST,
        db_name = KRAKEN_DB_NAME
    shell:
        r'''
        mkdir -p {OUTPUT_DIR}/2a_kraken2
        db_path="{params.db_host}"
        db_name="{params.db_name}"
        printf "kraken_db_path\t%s\n" "$db_path" > {output.info}
        printf "kraken_db_name\t%s\n" "$db_name" >> {output.info}
        '''

# Kraken2 contamination 요약표 (sample x top1/top2/top3/unclassified)
rule kraken2_make_summary_table:
    input:
        kreports = expand(f"{OUTPUT_DIR}/2a_kraken2/{{sample}}.kreport", sample=SAMPLES),
        db_info  = f"{OUTPUT_DIR}/2a_kraken2/kraken2_db_info.txt"
    output:
        table = f"{OUTPUT_DIR}/2a_kraken2/kraken2_summary.csv"
    run:
        import csv, os
        ...
        for sample in SAMPLES:
            ...
            top3s = fmt_species(top3[2] if len(top3) > 2 else None)
        ...
        with open(output.table, "w", newline="") as out:
            w = csv.DictWriter(out, fieldnames=["Sample", "Top1", "Top2", "Top3", "Unclassified"])
            w.writeheader()
            w.writerows(rows)
...
rule summary_report:
    input:
        mlst_table  = f"{OUTPUT_DIR}/2_ST_srst2_out/mlst_master.csv",
        mlst_done   = f"{OUTPUT_DIR}/2_ST_srst2_out/mlst_srst2.done.txt",
        kraken2_table = f"{OUTPUT_DIR}/2a_kraken2/kraken2_summary.csv",
        kraken2_db_info = f"{OUTPUT_DIR}/2a_kraken2/kraken2_db_info.txt",
        args_done   = f"{OUTPUT_DIR}/3_ARGs_srst2_out/args_srst2.done.txt",
        plas_done   = f"{OUTPUT_DIR}/4_Plasmid_srst2_out/plasmid_srst2.done.txt",
    ...
    run:
        ...
        kraken2_table  = os.path.abspath(input.kraken2_table)
        tetyper_json   = os.path.abspath(input.tetyper_json)
        master_csv     = os.path.abspath(input.master_csv)
        kraken2_db_name = KRAKEN_DB_NAME or os.path.basename(os.path.abspath(KRAKEN_DB.rstrip("/")))
        try:
            with open(input.kraken2_db_info) as fh:
                for line in fh:
                    k, v = (line.rstrip("\n").split("\t", 1) + [""])[:2]
                    if k == "kraken_db_name" and v.strip():
                        kraken2_db_name = v.strip()
                        break
        except Exception:
            pass
        kraken2_db_name_esc = kraken2_db_name.replace("'", "\\'")
        report_rmd_esc = str(REPORT_RMD).replace("'", "\\'")
          ...
          Rscript -e "candidates <- c('{report_rmd_esc}', '/pipeline/scripts/20251007_Summary_WGS_tem_v3.Rmd', '/home/uhlemann/heekuk_path/GoWGS/scripts/20251007_Summary_WGS_tem_v3.Rmd'); \
            ...
            rmarkdown::render(tmp, \
              ...
              params=list( \
                ...
                kraken2_table='{kraken2_table}', \
                kraken2_db_name='{kraken2_db_name_esc}' \
              ))"
        """)
